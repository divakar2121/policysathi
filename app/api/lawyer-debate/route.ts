...
Generate detailed debate with many argument turns, then final verdict as JSON.`;
    ...
    const saveDebate = async (debate: unknown, verdict: unknown) => {
      try {
        if (supabase) {
          ...
          if (error) {
            log('ERROR', 'Failed to save debate', { error: error.message });
          } else {
            log('INFO', 'Debate saved to database');
          }
        }
      } catch (dbError) {
        log('ERROR', 'Database save error', { error: String(dbError) });
      }
    };
    ...
    try {
      ...
      // Save in background
      saveDebate(parsed.debate, parsed.verdict);
      ...
      if (match) {
        try {
          ...
          // Save extracted debate in background
          saveDebate(extracted.debate, extracted.verdict);
          return Response.json(extracted);
        } catch (extractError) {
          log('ERROR', 'Failed to parse extracted JSON', { error: String(extractError) });
        }
      }
      ...
      // Save fallback in background
      saveDebate(fallbackDebate, fallbackVerdict);
